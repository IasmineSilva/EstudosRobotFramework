...
${ISSUES_URI}       /repos/iasminesilva/estudos-robot-framework/issues
...
Solicitar os dados do meu usuário
    ${MY_USER_DATA}     Get      alias=mygithubAuth   uri=/user
...
Pesquisar issues com o state "${STATE}" e com o label "${LABEL}"
    ...
    ${MY_ISSUES}        Get          alias=mygithub       uri=${ISSUES_URI}    params=${PARAMS}
...
Enviar a reação "${REACTION}" para a issue "${ISSUE_NUMBER}"
    ...
    ${MY_REACTION}      Post     alias=mygithubAuth    uri=${ISSUES_URI}/${ISSUE_NUMBER}/reactions
